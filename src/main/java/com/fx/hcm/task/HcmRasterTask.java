 ...
 * @author pscha
 */
public class HcmRasterTask extends Task<Integer> {

    private static final Logger _log = LogManager.getLogger(HcmRasterTask.class);
    ...
    public HcmRasterTask(ProgressDialog progressDialog, File files[], JXMapViewer mapViewer, List<Painter<JXMapViewer>> painters, TableView<Topo> tableView, Color color, TableView<Dir> tableViewDir, Dir dir) {
    ...
    protected Integer call() throws Exception {
        ...
        for (File file : files) {
            if (file.isDirectory()) {
                File hcmFiles[] = file.listFiles();
                for (File hcmFile : hcmFiles) {
                    if (!hcmFile.isDirectory()) {
                        rasterList.add(new SrtmRaster(hcmFile.getName()));
                    }
                }
            }
        }
